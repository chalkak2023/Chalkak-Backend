...
<details>
  <summary>[동시성 처리] BullQueue</summary>
  <div markdown="1">
    <ul>
      <li>도입배경</li>
        <ul>
        ...
        <ul>
          ...
          <li>Kafka</li>
            <ul>
              <li>장점</li>
                <ul>
                  <li>높은 처리량과 낮은 지영 시간을 제공하며 분산 스트리밍 처리에 적합하다. 그리고 데이터를 병렬로 처리하기 때문에 처리량이 높다.</li>
                </ul>
              <li>단점</li>
                <ul>
                  <li>주로 대용량 처리를 위해 사용된다. 상대적으로 복잡하고 운영 관리가 어려울 수 있다.</li>
                </ul>
            </ul>
        </ul>
      <li>최종결정</li>
...
<details>
  <summary>[이미지 스토리지] AWS S3</summary>
  <div markdown="1">
    <ul>
      <li>도입배경</li>
        <ul>
          <li>사진을 많이 저장되면 스토리지 공간을 확장해야 한다. </br>그렇기 때문에 스토리지 공간을 필요한 만큼 동적으로 할당하여 효율적으로 사진을 관리할 수 있는 클라우드 기반의 객체 스토리지 서비스가 필요하다.</li>
        </ul>
      <li>기술비교</li>
        <table width="70%">
          <thead>
            ...
            <ul>
              <li>장점</li>
                <ul>
                  ...
                  <li>지리적으로 분산된 데이터 복제 기능을 제공하여 데이터 손실을 방지한다.</li>
            ...
            <ul>
              <li>장점</li>
                <ul>
                  ...
                  <li>Azure Portal, PowerShell, CLI를 사용하여 구성 및 관리 가능하다.</li>
                </ul>
              <li>단점</li>
                <ul>
            ...
            <ul>
              <li>장점</li>
                <ul>
                  ...
                  <li>지리적으로 분산된 데이터 복제 기능을 제공하여 데이터 손실을 방지한다.</li>
                  <li>AWS Management Console, CLI, SDK를 사용하여 구성 및 관리 가능하다.</li>
                </ul>
              <li>단점</li>
                <ul>
      ...
      <li>최종결정</li>
        <ul>
          <li>저렴한 비용과 AWS에서 RDS, EC2등 AWS서비스를 같이 사용하기 때문에 비용관리가 용이하고 접근하고 사용하기 쉬운 API와 SDK를 제공한다. </br>그리고 자세하고 이해하기 쉽게 정리가 잘 되어있는 문서를 제공하기 때문에 결정하게 되었다.</li>
        </ul>
    </ul>
  </div>
</details>
            ...
            <ul>
              ...
              <li>단점</li>
                <ul>
                  ...
                  <li>테이블 간의 관계가 복잡해질 수 있다 데이터베이스 디자인이 보다 어려워진다.</li>
                </ul>
            </ul>
        </ul>
      <li>최종결정</li>
        <ul>
          <li>Multi Table Inheritance를 사용하면 조인이 복잡해지고 조회 성능이 느려진다. </br>유저를 가리키는 외래키가 복합키가 되어 회원이 여러 관계의 중심이 될 수 있으므로, 복잡성이 증가한다.</br>따라서 싱글 테이블 전략을 선택해서 조인을 단순화시켜 조회 성능을 올리고 연관성과 복잡성을 줄이기로 결정했다.</li>
        </ul>
    </ul>
  </div>
</details>
...
<details>
  ...
  <div markdown="1">
    <ul>
      <li>도입배경</li>
        <ul>
          <li>직접 테스트 코드를 실행하고 수동으로 배포를 하는 번거로움이 있다. 이 과정을 자동으로 만들면 개발에 시간을 더욱 할애할 수 있다.</li>
        </ul>
      <li>기술비교</li>
        <table width="70%">
          <tr>
          ...
          <tr>
            <td>설치 및 설정</td>
            <td>GitHub 저장소 내에서 워크플로우 파일 작성으로 간단히 설정 가능</td>
        ...
        <ul>
          <li>Jenkins</li>
            <ul>
              <li>장점</li>
                <ul>
                  <li>매우 높은 사용자 지정성을 제공한다.</li>
                  <li>다양한 플러그인과 인터페이스를 지원한다.</li>
                  <li>매우 넓은 커뮤니티를 지원 받는다.</li>
                </ul>
              <li>단점</li>
                <ul>
                  <li>설정이 쉽지 않다.</li>
          ...
          <li>Travis CI</li>
            <ul>
              <li>장점</li>
                <ul>
                  <li>쉽게 설정할 수 있다.</li>
                  <li>매우 높은 호스팅 서비스를 제공한다.</li>
                  <li>커스텀 빌드 환경을 지원한다.</li>
                </ul>
              <li>단점</li>
                <ul>
                  <li>보통의 사용자 지정성이 제공한다.</li>
                  <li>비싼 요금이 발생할 수 있다.</li>
                  <li>보통의 커뮤니티 지원을 받는다.</li>
                </ul>
            </ul>
            ...
            <ul>
              ...
              <li>단점</li>
                <ul>
                  ...
                  <li>사용자 지정성이 다른 도구에 비해 낮다.</li>
                  <li>상대적으로 새로운 기술이기 때문에 자료가 부족하고 커뮤니티가 부족하다.</li>
                </ul>
            </ul>
        </ul>
      <li>최종결정</li>
        <ul>
          <li>비용이 무료이면서 우리가 사용하고 있는 GitHub와 연동이 매우 원활하고 접근성이 좋은 GitHub Action을 이용해서 CI/CD를 도입할 수 있도록 결정했다.</li>
        </ul>
    </ul>
  </div>
</details>
...
<details>
  <summary>유저 권한의 검증 문제</summary>
  <div markdown="1">
    <ul>
      <li>발생 문제</li>
        <ul>
          <li>유저 권한을 검증하는 Passport를 사용한 AuthGuard를 통과하면, 매번 DB 조회를 해서 유저의 블락 여부를 확인했다. </br>하지만 매번 DB 조회를 하기 때문에 리소스 낭비가 발생하고, 유저가 많을수록 성능이 떨어졌다.</li>
        </ul>
      <li>해결과정</li>
        <ul>
          <li>매 API 요청마다 DB 조회를 하지 않기 위해 CacheManager로 5분 동안 가져온 블락 여부를 저장한다. </br>이를 통해 DB 조회 횟수를 줄여서 리소스를 효율적으로 활용하고, 성능을 개선할 수 있다.</li>
        </ul>
...
<details>
  <summary>Redis 부하 문제</summary>
  <div markdown="1">
    <ul>
      <li>발생 문제</li>
        <ul>
          <li>API를 요청한 모든 회원의 블락 여부를 저장하다보니 레디스의 부하가 심해진다.</li>
        </ul>
      <li>해결과정</li>
        <ul>
          <li>블락된 유저의 정보를 캐시에 저장하고, 액세스 토큰의 만료 시간을 1시간으로 지정한다. </br>이를 통해 최근 1시간 동안 블락된 유저 숫자만큼만 캐시에 올라가게 되어, 캐시 성능이 개선된다. </br>또한, 액세스 토큰이 만료되면 로그아웃 처리하고, 로그인을 막는다. </br>이를 통해 보안성을 높이고, 블락 여부 조회를 DB 조회와 함께 처리하여 DB 조회 횟수를 최소화한다.</li>
        </ul>
      <li>수치 비교</li>
        <ul>
          <li>유저수가 늘수록 비례해서 증가하던 기존 메모리 사용구조 ⇒ 최대 100명 수준을 넘지 않는 메모리 사용구조</li>
...
<details>
  <summary>큰 이미지 처리 문제</summary>
  <div markdown="1">
    <ul>
      <li>발생 문제</li>
        <ul>
          <li>이미지의 크기가 크면 업로드하는 속도가 느리고 웹 페이지 로딩 속도가 느려진다.</li>
        </ul>
      <li>해결과정</li>
        <ul>
          <li>sharp를 이용해서 큰 사이즈의 사진을 찰칵 웹 페이지에서 확인하기 부족하지 않을 정도의 적당한 크기로 리사이징해서 사이즈를 대폭 줄인다.</li>
        </ul>
      <li>결과</li>
        <ul>
          ...
          <li>사진 SIZE 160324를 72626으로 54.7% 감소</li>
            <ul>
              <li>리사이징 전</li>
...
### 즉각적인 BullQueue 작업결과 응답

<details>
  <summary>요청 결과가 잘못되는 문제</summary>
  <div markdown="1">
    <ul>
      <li>발생 문제</li>
        <ul>
          <li>정상적인 요청 결과를 사용자가 받지 못 한다. </br>Job을 대기열에 추가한 후 사용자는 즉시 응답을 받으며 작업이 끝날 때까지 기다리지 않는다</li>
          <li>정상적인 요청 결과를 사용자가 받지 못 한다. </br>Job을 대기열에 추가하여 참여 요청을 처리하는 것과 별개로 사용자는 항상 성공했다는 결과만 받는 현상 발생 (성공/실패 여부와 상관없이 항상 성공 결과 리턴)</li>
        </ul>
      <li>해결과정</li>
        <ul>
          <li>작업이 완료될 때까지 잠시 기다린 후 응답을 반환하기 위해 메서드 간에 Event-Emitter를 사용한다.</li>
        </ul>
      <li>결과</li>
        <ul>
          <li>해결 전 - Bull Queue만 사용</br>15, 16번 유저만 참여에 성공했지만 14, 17, 18번 유저도 201 성공 결과를 반환받고 있다.</li>
          <img src="./docs/트러블슈팅/bullqueue/1.png" width=70%>
          <img src="./docs/트러블슈팅/bullqueue/2.png" width=70%>
          <li>해결 후 - Event-Emitter 사용</br>14, 15번 유저가 성공했고 16, 17, 18번 유저는 실패하여 403 정원이 다 찼다는 실패 결과를 반환받고 있다.</li>
